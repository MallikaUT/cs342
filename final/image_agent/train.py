...
def train(args):
    ...
    train_data = load_detection_data(
        train_dir, num_workers=0, batch_size=batch_size)
    valid_data = load_detection_data(
        valid_dir, num_workers=0, batch_size=batch_size)
    ...
    for epoch in range(args.epochs):
        ...
        for img, label in train_data:
            ...
            label_resized = torch.nn.functional.interpolate(label_resized, size=(logit_reshaped.shape[2], logit_reshaped.shape[3]), mode='nearest').squeeze(1)
